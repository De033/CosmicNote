...
网站的视觉外观由 CSS 控制，你目前不能在 Typst 中直接改变文字的 #text(fill: blue)[颜色]、#text(size: 14pt)[大小] 或 #text(font: "LXGW Bright")[字体] 等样式。如需修改，请在 `assets/custom.css` 文件中添加自定义 CSS 样式。

```typ
你目前不能在 Typst 中直接改变文字的 #text(fill: blue)[颜色]、#text(size: 14pt)[大小] 或 #text(font: "LXGW Bright")[字体] 等样式。
```
